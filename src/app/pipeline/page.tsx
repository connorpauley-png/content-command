...
import { Plus, GripVertical, Trash2, Sparkles, Filter, Trash, ImagePlus, FolderOpen, Camera, Loader2, CheckCircle, XCircle, Image, CalendarClock, RefreshCw, MousePointerClick, CheckSquare } from 'lucide-react'
...
const columns: { id: PostStatus; label: string; color: string }[] = [
  ...
  { id: 'needs_photos', label: '📷 Needs Photos', color: 'border-orange-500/50' },
...
function PostCard({ post, onClick, accountName, selectMode, selected, onToggleSelect, onRetryPublish }: { post: Post; onClick: () => void; accountName: string; selectMode?: boolean; selected?: boolean; onToggleSelect?: () => void; onRetryPublish?: () => void }) {
...
function Column({ id, label, color, posts, onAddPost, onClickPost, getAccountName, extraAction, selectMode, selectedIds, onToggleSelect, onSelectAllInColumn, onRetryPublish }: {
      ...
      <div className={cn('rounded-lg border-t-2 bg-muted/30 h-full', color, isOver && 'bg-primary/5')}>
        ...
        <div ref={setNodeRef} className="p-2 space-y-2 min-h-[200px]">
          <SortableContext items={posts.map((p) => p.id)} strategy={verticalListSortingStrategy}>
            {posts.map((post) => (
              <PostCard key={post.id} post={post} onClick={() => onClickPost(post)} accountName={getAccountName(post)} selectMode={selectMode} selected={selectedIds?.has(post.id)} onToggleSelect={() => onToggleSelect?.(post.id)} onRetryPublish={onRetryPublish ? () => onRetryPublish(post.id) : undefined} />
            ))}
          </SortableContext>
        </div>
      </div>
      ...
      {photoSourcePost && (
        <PhotoSourceDialog
          ...
          onPhotosSelected={(urls) => {
            ...
            movePost(photoSourcePost.id, 'approve_photos')
